...
const PlusMinusInput = ({
  ...
  return (
    ...
    >
      {label && (
        <span className={`text-[15px] ${isDetails ? "!text-base" : ""}`}>
          Količina:&nbsp;
        </span>
      )}
      <div
        className={`flex h-5 items-stretch justify-between rounded-[2px] bg-[#E9E9E9] px-1 ${isDetails ? "w-28 py-1" : "w-[68px] p-0"}`}
      >
        <button
          className={`flex shrink-0 cursor-pointer items-center justify-center text-[15px] ${isDetails ? "w-8 !text-gray-400" : "w-4"}`}
          onClick={() =>
            countQuantity({
        ...
        <input
          type={`text`}
          className={`w-full border-0 bg-inherit text-center text-[15px] font-normal focus:outline-none focus:ring-0 ${isDetails ? "py-1 !text-gray-400" : "p-0"}`}
