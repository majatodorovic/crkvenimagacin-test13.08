...
const NavigationMobile = () => {
  ...
  const categoriesMain = [
    { name: "Početna", slug: "/" },
    { name: "Blog", slug: "/vesti" },
    { name: "Kontakt", slug: "/kontakt" },
    { name: "Na popustu", slug: "/na-popustu" },
  ];
  ...
  const [expandedTopNav, setExpandedTopNav] = useState(null);
  ...
  return (
    <>
      ...
      >
        <form
          className="absolute mx-auto mt-14 flex h-12 w-[80%] items-center py-2"
          onClick={() => setSearchOpen(true)}
        >
          <div
            type="text"
            className="h-full w-full border border-primary bg-transparent py-2 pl-8 text-xs text-black text-white mix-blend-difference placeholder:text-xs placeholder:text-black focus:border-primary focus:outline-none focus:ring-0"
            placeholder="Pretraga"
            onChange={(e) => setSearchTerm(e.target.value)}
            onMouseDown={() => setSearchOpen(true)}
          />
          <p className="absolute left-8 text-sm text-black">Pretraga</p>
          <i className="fa-solid fa-search absolute left-2 top-5 text-xs text-black"></i>
        </form>
      </div>
      <div
        className={
      ...
      >
        ...
        <div className="mx-auto mt-5 flex w-[95%] flex-row gap-1 overflow-x-auto border-b pb-1">
          {topNavItems.map((item) => (
            <div
              key={item.id}
              className={`w-fit cursor-pointer whitespace-nowrap px-1 py-1 text-base text-black ${
        ...
        <div className="fixed left-0 top-0 z-[10000] h-screen w-screen overflow-y-auto bg-white">
          <div className="mx-auto mt-6 flex w-[95%] items-center gap-2">
            <form onSubmit={handleSearch} className="relative w-full">
              <input
                type="text"
                className="w-full rounded-[10px] border border-gray-400 pl-10 placeholder:text-base focus:border-gold focus:outline-none focus:ring-0"
