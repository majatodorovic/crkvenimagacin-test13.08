...
const SinglePost = ({ post }) => {
  ...
  return (
    <div className="sectionPaddingX">
      <div className="sectionPaddingX relative flex gap-8 bg-white pt-6 max-xl:flex-col lg:pt-10 2xl:pt-14">
        <div className={`flex w-full flex-1 flex-col gap-3`}>
          <p
            className={`w-fit bg-primary px-2 text-base font-light text-white`}
          >
            {new Date(
        ...
        >
          {blogGallery?.map((image, index) => {
            return (
              <Slide key={index} className={`overflow-hidden bg-lightGray`}>
                <div className={`relative`}>
                  <Image
                    ...
                    sizes={`100vw`}
                    priority
                    className={`aspect-[14/9] h-full w-full bg-lightGray object-cover`}
                  />
                </div>
              </Slide>
            );
